...
#'
#' This function allows downloading and processing the fastq samples in a CSV file by using salmon aligner. This funtion is used internally by \code{\link{rawAlignment}} but it can be used separatelly. Furthermore, the function can download the reference files required: FASTA Reference Genome and GTF file.
...
salmonAlignment <- function(data,downloadRef=FALSE,downloadSamples=FALSE, createIndex = TRUE, BAMfiles = TRUE, SAMfiles = TRUE, countFiles = TRUE,referenceGenome = 38,customFA = "", customGTF = "",tx2Counts = tx2Counts){
  ...
  cat("Using salmon sequencing tool...\n")
  ...
  if(!is.null(data$Run)){
    ...
    for(i in seq_len(dim(samples)[1])){
      ...
      cat(paste("File succesfully stored at ",paste(" ReferenceFiles/Samples/RNAseq/CountFiles/", samples[i,]$Run,"/",samples[i,]$Run, ".count",sep = "")))
    }
    ...
    for(i in seq_len(dim(samples)[1])){
      ...
      cat(paste("File succesfully stored at ",paste(" ReferenceFiles/Samples/RNAseq/CountFiles/", samples[i,]$Comment.ENA_RUN.,"/",samples[i,]$Comment.ENA_RUN., ".count",sep = "")))
