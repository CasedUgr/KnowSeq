...
#' @examples
#' geneExpressionMatrix <- calculateGeneExpressionValues(countsMatrix, annotation, genesNames=TRUE)
#' geneExpressionMatrix <- calculateGeneExpressionValues(countsMatrix, annotation, genesNames=FALSE)
...
calculateGeneExpressionValues <- function(countsMatrix,annotation,genesNames=TRUE,notHuman = FALSE, notHumanGeneLengthCSV = ""){
  ...
  if(genesNames){
    rownames(expressionMatrix) <- annotation$external_gene_name[which(annotation$ensembl_gene_id == mygenes)]
